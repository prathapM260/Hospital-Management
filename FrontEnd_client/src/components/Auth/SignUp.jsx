  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      Swal.fire({
        title: "Error",
        icon: "error",
        text: err.response?.data?.message || "Error Registering User! Please Try Again!",
      });
    }
  };

  return (
    <motion.section className="bg-[#cbf5f6]">
      <motion.div
        ref={ref}
      ...
      >
        <div className="xl:mx-auto xl:w-full xl:max-w-sm 2xl:max-w-md bg-[#CCD5AE] shadow-xl shadow-black p-5 rounded-lg mt-6">
          ...
          <p className="mt-2 text-center text-base text-gray-600">
            ...
            <Link to="/sign-in" className="font-medium text-black transition-all duration-200 hover:underline">
              Sign In
            </Link>
          </p>
          <form className="mt-8">
            <div className="space-y-5">
              <div>
                ...
                <div className="mt-2">
                  <input
                    className="flex h-10 w-full rounded-md border border-black bg-transparent px-3 py-2 text-sm placeholder:text-black focus:outline-none focus:ring-1 focus:ring-gray-400 focus:ring-offset-1"
                    type="text"
